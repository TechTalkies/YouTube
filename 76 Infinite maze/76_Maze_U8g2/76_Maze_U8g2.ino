...
const int thresh = 2500;  //Tilt threshold
...
// ----- Pins -----
#define BUTTON_PIN 0  // press to reset (BOOT button on many ESP32 dev boards)
...
void loop() {
  ...
  if (digitalRead(BUTTON_PIN) == LOW && (millis() - lastBtnMs) > 250) {
    lastBtnMs = millis();
    // wait release (optional but helps avoid multiple resets)
  ...
  // axes flipped: gyro x -> Y movement, gyro y -> X movement
  int dx = 0, dy = 0;
  if (x > 10 && x < 200) dx = 1;
  if (x > 250 && x < 350) dx = -1;
  if (y > 10 && y < 200) dy = 1;
  ...
  // try to move only if target cell is open (maze==1)
  int newX = ballX + dx;
  int newY = ballY + dy;
  ...
  delay(50);  // movement speed / debounce feel 110
}