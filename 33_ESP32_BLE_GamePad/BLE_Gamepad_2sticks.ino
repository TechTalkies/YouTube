...
#define VLX_JOYSTICK 0 //Change
#define VLY_JOYSTICK 0 //Change
...
uint16_t VlxReading = 0;
uint16_t VlyReading = 0;
uint16_t VlxValue = 0;
uint16_t VlyValue = 0;
...
void loop() {
  if (bleGamepad.isConnected()) {
    ...
    VlxReading = analogRead(VLX_JOYSTICK);
    VlyReading = analogRead(VLY_JOYSTICK);

    VrxValue = map(VrxReading, 4095, 0, 0, 32737);
    VryValue = map(VryReading, 4095, 0, 0, 32737);

    VlxValue = map(VlxReading, 4095, 0, 0, 32737);
    VlyValue = map(VlyReading, 4095, 0, 0, 32737);
    ...
    bleGamepad.setRightThumb(VlxValue, VlyValue);
